...
class SecureLoginService {
  ...
  verifyToken(rawToken) {
    ...
    const user = this.userStore.getUser(record.user_id);
    if (!user || user.revoked) throw new Error('Login user is not active or unavailable');
    return record.user_id;
  }

  redeemToken(rawToken) {
    const tokenHash = crypto.createHash('sha256').update(rawToken).digest('hex');
    return this.userStore.redeemLoginToken({ tokenHash, now: this.now() });
  }

  completeLogin(rawToken, persistSession) {
    if (typeof persistSession !== 'function') throw new TypeError('persistSession is required');
    const tokenHash = crypto.createHash('sha256').update(rawToken).digest('hex');
    return this.userStore.redeemLoginToken({
      tokenHash,
      now: this.now(),
      onAuthorized: persistSession,
    });
  }
}
